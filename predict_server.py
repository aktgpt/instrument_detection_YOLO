...
def _main_(args):
    ...
    video_files = glob.glob(video_folder_path + '*.mp4')
    video_out_folder = 'predictions/'
    ...
    for i in range(len(video_files)):
        image_path = video_files[i]
        filename = os.path.splitext(os.path.basename(image_path))[0]
        if image_path[-4:] == '.mp4':
            video_out = video_out_folder + filename + '_detected_tiny_yolo' + image_path[-4:]
...
if __name__ == '__main__':
    args = argparser.parse_args(['-c', 'config.json', '-w', 'best_saved_weights_tiny_yolo.h5', '-i', 'Test/'])
    _main_(args)
